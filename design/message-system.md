...
この仕組み全体がメッセージシステムであり、基本的には、Characterが行動を起こす度に、返されたメッセージがあればそれを処理していって、最終的にメッセージを返さなくなるまで処理を続ける、という、全体としてはシンプルな仕組みになる。
...
- Characterが行動を行う
- 行動結果としてメッセージが発生した場合、そのメッセージがメッセージシステムを経由して対象のCharacterにメッセージを送る
- メッセージを送られたCharacterは、さらにメッセージを返すことができる。メッセージが返されなくなるまでこの処理は継続する。
