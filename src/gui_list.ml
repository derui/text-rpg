...
type _operation = Up | Down | Select 
...
module T = struct
  ...
  let handle_operation t ~operation =
    let t = match operation with
      | Up -> move_cursor t `Up
      | Down -> move_cursor t `Down
      | Select -> t
    in
    Lwt.return t
...
let make ~items ~rect ~item_height ~font =
  (module struct
    module Gui = T
    let this = {T.items = items; rect; item_height; font; current = 0} 
  end : G.Gui_instance)
